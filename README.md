...
The hunt shares the box with the Minecraft server, so it is fenced at the kernel level: a cpuset keeps it off physical cores 0-1 entirely, and its own slice at `CPUWeight=1` makes Minecraft win 100:1 on the cores they share. Nice alone is not enough because Minecraft runs in a screen under user.slice and nice does not cross cgroups.

Check the hyperthread pairs. The i7-8700 prints `0,6 1,7 2,8 3,9 4,10 5,11`; if yours prints `0,1 2,3 ...` use `AllowedCPUs=4-11`:
...
- A "deferred N hairline crossing(s)" line means the base pass saw a hairline sign change the accurate kernel could not confirm, so the spot was handed to the dip probes: a real sub-lattice pair is recovered exactly there (shows up in probes=c/r), a true phantom resolves to nothing. Routine in small numbers; this keeps false crossings out of the ledger, where a phantom pair could otherwise mask a genuinely missing pair in the same certified stretch.
...
- A TURING SURPLUS line means more crossings were counted than zeros exist. That is a bug in the program not a discovery. The block gets the 8x pass first; if the extra crossing survives it, the line is written, the ledger is re-based to the certified count so the phantom cannot mask a later deficit, and the block should be checked with `./riemann check`.
